import { Trophy, Target, Flame, Zap, Calendar, BookOpen, Star, Award, Lock, Heart, Diamond, Shield, Crown, TrendingUp } from "lucide-react";
...
const TIER_COLORS: Record<string, string> = { bronze: "from-orange-600 to-orange-800", silver: "from-slate-400 to-slate-600", gold: "from-yellow-500 to-amber-600", diamond: "from-cyan-400 to-blue-600" };
...
const StatsSection = () => {
  ...
  return (
    <div className="space-y-4">
      {/* XP Level Progress */}
      <FadeIn delay={0.05}>
        <div className="bg-card rounded-2xl p-4 border border-border/40 shadow-card">
        ...
        <div className="bg-card rounded-2xl p-3.5 border border-border/40 shadow-card">
          ...
          <div className="flex items-center gap-2 mb-2">
            {Array.from({ length: stats.maxHearts }).map((_, i) => (
              <motion.div key={i} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.2 + i * 0.06, type: "spring" }}>
                <Heart size={24} className={i < stats.hearts ? "text-agni-pink fill-agni-pink" : "text-muted-foreground/30"} />
              </motion.div>
            ))}
          </div>
          {stats.hearts < stats.maxHearts && (
            <motion.button whileTap={{ scale: 0.95 }} onClick={() => refillHearts(true)}
      ...
      <FadeIn delay={0.2}>
        <div className="bg-card rounded-2xl p-3.5 border border-border/40 shadow-card">
          ...
          <div className="flex justify-between gap-1 mb-3">
            {streakDays.map((d, i) => (
              <motion.div key={i} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.25 + i * 0.04, type: "spring" }} className="flex flex-col items-center gap-1">
          ...
          <motion.button whileTap={{ scale: 0.95 }} onClick={buyStreakFreeze}
            ...
            <Shield size={10} /> Buy Streak Freeze <Diamond size={8} className="text-agni-gold ml-1" /> <span className="text-agni-gold">{STREAK_FREEZE_COST}</span>
          </motion.button>
        </div>
      </FadeIn>
      ...
      <FadeIn delay={0.3}>
        <div className="bg-card rounded-2xl p-3.5 border border-border/40 shadow-card">
          ...
          <div className="flex items-center gap-3 p-3 bg-gradient-to-r from-agni-purple/10 to-agni-blue/10 rounded-xl border border-agni-purple/20">
            <motion.span className="text-3xl" animate={{ scale: [1, 1.15, 1] }} transition={{ duration: 2, repeat: Infinity }}>{league.emoji}</motion.span>
            <div className="flex-1">
          ...
          <div className="flex justify-between mt-3 px-2">
            {[
              ...
              <motion.div key={tier.name} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 + i * 0.05 }}
        ...
        <div className="bg-card rounded-2xl p-3.5 border border-border/40 shadow-card">
          ...
          <div className="grid grid-cols-4 gap-2">
            {MODULE_PROGRESS.map((mod, i) => {
              ...
              return (
                <motion.div key={mod.title} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4 + i * 0.08 }} className="flex flex-col items-center">
      ...
      <FadeIn delay={0.4}>
        <div className="bg-card rounded-2xl p-3.5 border border-border/40 shadow-card">
          ...
          <div className="w-full h-2 bg-muted/30 rounded-full mb-3 overflow-hidden">
            <motion.div className="h-full bg-gradient-to-r from-agni-gold to-agni-orange rounded-full"
              initial={{ width: 0 }} animate={{ width: `${(unlockedAchievements / achievements.length) * 100}%` }} transition={{ duration: 0.8, delay: 0.5 }} />
          </div>
          <div className="grid grid-cols-3 gap-2">
            {achievements.map((a, i) => (
              <motion.div key={a.id} initial={{ opacity: 0, scale: 0.7, rotateY: 90 }} animate={{ opacity: 1, scale: 1, rotateY: 0 }}
                transition={{ delay: 0.5 + i * 0.04, type: "spring", stiffness: 200 }}
                className={`relative rounded-xl p-2 text-center border transition-all ${
                  a.unlocked ? `bg-gradient-to-b ${TIER_COLORS[a.tier]}/5 border-agni-gold/30` : "bg-muted/20 border-border/30"
                }`}>
                {!a.unlocked && <div className="absolute top-1 right-1"><Lock size={7} className="text-muted-foreground/40" /></div>}
                <motion.span className={`text-lg block ${!a.unlocked ? "grayscale opacity-40" : ""}`}
                  animate={a.unlocked ? { scale: [1, 1.1, 1] } : {}} transition={{ duration: 0.5, delay: 0.6 + i * 0.04 }}>{a.emoji}</motion.span>
        ...
        <div className="bg-card rounded-2xl p-3.5 border border-border/40 shadow-card">
          ...
          <div className="space-y-2">
            {[
              ...
              <motion.div key={item.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.06 }}
